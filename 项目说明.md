...
- 每日配额：100次提问
- 文件大小限制：10MB
- 相似度阈值：0.5（不做阈值降级；检索为空直接走无上下文兜底提示。依据 text-embedding-v3 实测余弦分布：相关 0.55~0.74、无关 0.40~0.42，0.5 为干净分界，详见架构图与 RAG 阈值小节）
...
> 说明：本地 bge 离线 embedding 方案已移除，Embedding 仅用云端 DashScope `text-embedding-v3`。LLM 调用经 `FallbackLLM` 封装，仍保留 `OllamaLLM`（`qwen2:7b`）作为**可选备用分支**，但需本地部署并运行 Ollama 才生效；默认环境未运行 Ollama，故**实际仅云端 DashScope 生效**，部署复杂度与依赖体积维持在最低。
...
本项目使用了workbuddy,Devin开发,当前AI编程工具极大的提高了开发效率,但是AI对于需求的理解不够全面,也容易出现代码整体风格不一致,技术栈选型混乱等问题,开发的同时也要严格审查,修改代码前要确认好影响范围或整理好修改方案,并且要做好版本控制,防止改偏却无法回退的情况
