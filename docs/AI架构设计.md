...
### 单意图RAG设计
本项目采用**单意图RAG问答架构**，所有用户问题直接进入统一的检索→生成管道，不进行意图分类和路由。设计考量：
- **简化架构**：避免引入额外的意图识别模块，降低系统复杂度
- **聚焦核心**：专注于检索质量和回答准确性，而非多业务场景分流
- **成本控制**：减少LLM调用次数（无需意图分类调用）
- **维护性**：单一管道更易于调试和优化

**注意**：数据库中`sessions.intent_tag`和`messages.intent`字段为历史遗留，当前版本不使用，将在后续版本清理。
