...
4. **Token 过期时间**：24 小时
5. **相似度阈值**：默认 **0.5**，低于此值的检索结果会被过滤。取值依据实测分布（text-embedding-v3 余弦相似度）：相关查询 0.55~0.74、无关查询 0.40~0.42；0.5 为干净分界，既能拦截无关 query 又能保留相关召回。早期版本曾写 0.6，但会误杀产品咨询等相关块（0.55~0.56），故下调。**检索降级不再降低相似度阈值**（旧实现降到 0.3 会把无关内容 0.40~0.42 重新漏入上下文），改由「意图识别层」对越界/闲聊 query 走无上下文兜底提示，从路由层杜绝无关内容注入；如需受限召回兜底，可配置 `retrieval_fallback_threshold`（建议 ≥0.45，须高于无关带）。
6. **Top-K 检索**：默认返回 8 个最相关的文档块
7. **`intent_tag` 字段**：由系统自动标注的可选字符串（如 `product_consult`），非固定枚举；查询/创建时可为 `null`
8. **意图识别与策略路由**：系统新增轻量意图门控层（详见《AI架构设计.md》§意图识别与策略路由）。规则分类器将 query 归类为 7 类业务意图（产品咨询/价格套餐/退款售后/账号登录/知识库文档/订单/兜底闲聊），置信度阈值 `intent_confidence_threshold`（默认 1.0，即至少命中一个有效业务词）以下的归为兜底闲聊。知识类意图走 RAG 主链路；兜底/未知意图直接走无上下文兜底提示（**不检索、不注入知识库内容**）。路由为纯函数单分发，无循环/遗漏风险；可经 `enable_intent_routing=false` 退化为纯 RAG。识别结果落库至 `messages.intent` 字段便于观测。
