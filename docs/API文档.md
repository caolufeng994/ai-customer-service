...
4. **Token 过期时间**：24 小时
5. **相似度阈值**：默认 **0.5**，低于此值的检索结果会被过滤。取值依据实测分布（text-embedding-v3 余弦相似度）：相关查询 0.55~0.74、无关查询 0.40~0.42；0.5 为干净分界，既能拦截无关 query 又能保留相关召回。早期版本曾写 0.6，但会误杀产品咨询等相关块（0.55~0.56），故下调。
